...
#[tokio::test]
async fn test_raft_3_node_quorum_consensus_and_disk_loss() {
    let dir1 = tempdir().unwrap();
    let dir2 = tempdir().unwrap();
    let dir3 = tempdir().unwrap();

    let addr1: SocketAddr = "127.0.0.1:41051".parse().unwrap();
    let addr2: SocketAddr = "127.0.0.1:41052".parse().unwrap();
    let addr3: SocketAddr = "127.0.0.1:41053".parse().unwrap();
    ...
    // 1. Client connects to Leader (Node 1) and pushes a message
    let stream1 = TcpStream::connect(addr1).await.unwrap();
    let (mut r1, mut w1) = stream1.into_split();

    let push_req = Request::Push {
        queue_name: "raft-replicated-q".to_string(),
        payload: b"mission-critical-data".to_vec(),
        delay_seconds: 0,
        message_id: String::new(),
    };
    codec::write_message(&mut w1, &push_req).await.unwrap();
    let push_resp: Response = codec::read_message(&mut r1).await.unwrap();

    let push_id = match push_resp {
        Response::Push { message_id } => message_id,
        other => panic!("expected push response, got {:?}", other),
    };
    assert!(!push_id.is_empty());

    // 2. DISK DESTRUCTION SIMULATION: Completely destroy Leader (Node 1) and its disk volume
    let _ = tx1.send(());
    drop(dir1);

    tokio::time::sleep(Duration::from_millis(100)).await;

    // 3. Promote Node 2 as new Leader
    raft2.recover_as_new_leader().await;

    // 4. Connect to new Leader (Node 2) -> Verify it holds replicated copy from Quorum!
    ...
    codec::write_message(&mut w2, &poll_req).await.unwrap();
    let poll_resp: Response = codec::read_message(&mut r2).await.unwrap();
    ...
    assert_eq!(msgs[0].payload, b"mission-critical-data");
}
